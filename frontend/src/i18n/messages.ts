...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '开始时间': 'Start time',
  '结束时间': 'End time',
  '开始时间（精确到分钟）': 'Start time (minute precision)',
  '结束时间（精确到分钟）': 'End time (minute precision)',
  '今天': 'Today',
  '昨天': 'Yesterday',
  '最近7天': 'Last 7 days',
  '最近30天': 'Last 30 days',
  ...
  '端点': 'Endpoint',
  '端点:': 'Endpoints:',
  '测试素材库连接': 'Test asset library connection',
  '正在测试素材库连接': 'Testing asset library connection',
  '选择素材库端点并测试': 'Select an asset library endpoint to test',
  '未配置可用的素材库端点': 'No active asset library endpoint is configured',
  '密钥已停用': 'Key is disabled',
  '请先配置并启用素材库端点': 'Configure and enable an asset library endpoint first',
  '连接测试失败': 'Connection test failed',
  '连接测试成功': 'Connection test passed',
  '素材库连接测试失败': 'Asset library connection test failed',
  '素材库连接正常': 'Asset library connection is healthy',
  '使用已保存的密钥发送只读请求，不会创建或删除素材。': 'Send a read-only request with the saved key. No assets will be created or deleted.',
  '测试密钥': 'Key under test',
  '素材库端点': 'Asset library endpoint',
  '选择素材库端点': 'Select an asset library endpoint',
  '测试将严格使用所选端点和当前密钥，不会自动切换。': 'The test uses only the selected endpoint and current key; it will not fail over.',
  '正在测试': 'Testing',
  '开始测试': 'Run test',
  '个素材组': 'asset groups',
  '号池': 'Pool',
  ...
  'API 格式': 'API formats',
  '高级认证参数': 'Advanced authentication parameters',
  '调度配置': 'Scheduling',
  '访问密钥 ID (AK)': 'Access Key ID (AK)',
  '访问密钥 Secret (SK)': 'Secret Access Key (SK)',
  '请输入 SK': 'Enter the Secret Access Key',
  '临时安全令牌': 'Temporary security token',
  '可选，使用临时凭据时填写': 'Optional; enter this when using temporary credentials',
  'AK 与 SK 留空表示保持原凭据；填写时必须成对更新。': 'Leave AK and SK empty to keep the existing credentials. Enter both when updating them.',
  '用于火山方舟素材库请求签名，AK 与 SK 必须成对填写。': 'Used to sign Volcengine Ark asset-library requests. AK and SK must be entered together.',
  'API 密钥': 'API key',
  '上游 API Key 请求头': 'Upstream API Key header',
  '仅用于 doubao:asset_library 的第三方 API Key 中转。': 'Used only for third-party API Key relays serving doubao:asset_library.',
  ...
  '用户错误': 'User errors',
  '已分类服务错误': 'Classified service errors',
  '最近服务错误': 'Recent service errors',
  '服务错误分布': 'Service error distribution',
  '服务错误趋势': 'Service error trend',
  '服务错误审计': 'Service error audit',
  '当前没有近期服务错误': 'No recent service errors',
...
const legacyPhraseEnglishMessages: Array<[string, string]> = [
  ...
  ['最近服务错误、熔断事件和可疑活动入口', 'Recent service errors, circuit breaker events, and suspicious activity entry points'],
  ...
  ['同步生图心跳已停用：错误会透传真实 HTTP 4xx/5xx 状态码', 'Synchronous image heartbeats are disabled so errors preserve their real HTTP 4xx/5xx status'],
  ['标准文本非流式心跳已停用：错误会透传真实 HTTP 4xx/5xx 状态码', 'Standard text non-streaming heartbeats are disabled so errors preserve their real HTTP 4xx/5xx status'],
