...
impl PromptCacheAffinitySettings {
    fn merge_from_value(&mut self, value: Option<&Value>) {
        ...
        if let Some(profiles) = settings.get("profiles").and_then(Value::as_array) {
            self.profiles = profiles.iter().filter_map(parse_affinity_profile).collect();
        }
            ...
            Some(Value::Object(auto)) => {
                if auto.get("enabled").and_then(Value::as_bool) == Some(false) {
                    ...
                    self.auto = Some(AutoAffinitySettings {
                        ...
                        prefix_chars: read_usize(
                            auto.get("prefix_chars"),
                            AUTO_PREFIX_CHARS_DEFAULT,
                        )
                        .max(1),
                    });
                }
...
pub(crate) async fn inject_provider_prompt_cache_affinity_key(
    ...
    let Some(cache_key) =
        settings.resolve_affinity_cache_key(requested_model, provider_request_body)
    else {
        return;
    };
...
mod tests {
    ...
    fn matches_profile_by_first_user_message_hash() {
        assert_eq!(
            resolved(
                &settings_value(true, "big prompt"),
                "glm-5.3",
                &chat_body("big prompt")
            ),
    ...
    fn disabled_or_unconfigured_is_passthrough() {
        assert_eq!(
            resolved(
                &settings_value(false, "big prompt"),
                "glm-5.3",
                &chat_body("big prompt")
            ),
            None
        ...
        assert_eq!(
            empty.resolve_affinity_cache_key("glm-5.3", &chat_body("big prompt")),
            None
        );
    }
    ...
    fn never_overrides_client_prompt_cache_key() {
        ...
        assert_eq!(
            resolved(&settings_value(true, "big prompt"), "glm-5.3", &body),
            None
        );
    }
