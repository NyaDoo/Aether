...
use crate::usage::spawn_sync_report_after_terminal_usage;
...
pub(crate) async fn submit_local_core_error_or_sync_finalize(
    ...
    submit_local_core_error_or_sync_finalize_with_candidate_gate(
        state, trace_id, decision, payload, true,
    )
    .await
}

/// Build the local-core response while carrying the result of the terminal
/// usage handoff into any background report generated by the response mapper.
/// The mapper may need to emit a report for provider-side effects, but an
/// unconfirmed usage handoff must not let that report publish a terminal
/// request-candidate status ahead of billing persistence.
pub(crate) async fn submit_local_core_error_or_sync_finalize_after_terminal_usage(
    state: &AppState,
    trace_id: &str,
    decision: &GatewayControlDecision,
    payload: GatewaySyncReportRequest,
...
async fn submit_local_core_error_or_sync_finalize_with_candidate_gate(
    state: &AppState,
    trace_id: &str,
    decision: &GatewayControlDecision,
    payload: GatewaySyncReportRequest,
