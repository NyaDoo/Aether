...
async fn lock_resolved_usage_for_cleanup(
    ...
    // SQLite has no SELECT ... FOR UPDATE.  A guarded no-op UPDATE acquires the
    // writer lock without changing retained evidence, closing the selection /
    // deletion race for the remainder of this transaction.
    let sql = format!(
        r#"
UPDATE "usage"
SET updated_at_unix_secs = updated_at_unix_secs
WHERE request_id = ?
  AND ({RESOLVED_USAGE_CLEANUP_PREDICATE})
"#
    );
